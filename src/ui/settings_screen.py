...
class SettingsScreen(BaseScreen):
    ...
    def _create_ui_elements(self):
        ...
        self.create_button(
            ...
            action=lambda: self._back_to_main_menu(),
            font_size='MEDIUM'
        )
    ...
    def draw(self):
        ...
        # Отрисовка заголовка
        title_text = self.fonts['LARGE'].render("Настройки", True, COLORS['PRIMARY_BLUE'])
    ...
    def _draw_volume_sliders(self):
        ...
        """
        # Параметры ползунков
        sound_label = self.fonts['MEDIUM'].render("Громкость звуков", True, COLORS['TEXT_DARK'])
        sound_label_rect = sound_label.get_rect(
            center=(self.width // 2, self.height // 3 - 30)
        )
        self.screen.blit(sound_label, sound_label_rect)
        
        music_label = self.fonts['MEDIUM'].render("Громкость музыки", True, COLORS['TEXT_DARK'])
    ...
    def _draw_language_selector(self):
        ...
        lang_label = self.fonts['MEDIUM'].render("Язык интерфейса", True, COLORS['TEXT_DARK'])
